...
# Compare the before and after of rules matching
...
model_name_list = ['logistic/baseline_hyper_and_energy.sav']#'logistic/majority_hyper_and_energy.sav']
...
for name in model_name_list:
	if name[-3:] =='.pt': # then its a knodle model and you need to load from cache:
		model = joblib.load('cache/'+name)
	else:
	...
	if model.normalizer == None:
		print('no norm')
		tensor_input = torch.from_numpy(model.data[model.feature_names].to_numpy())
	else:
		print('normalized', model.normalizer.transform(model.data[model.feature_names]))
		tensor_input = torch.from_numpy(model.normalizer.transform(model.data[model.feature_names]))
